	...
	<div class="con_main">
		<div class="childMain" style="width: 100%;">
			...
			<el-table ref="multipleTable" :data="tableData" v-loading.fullscreen.lock="loading2" @row-dblclick="dblclick" highlight-current-row @selection-change="selectChange" border style="width: 100%;" :height="tableHeight">
				...
				<el-table-column prop="C_NAME" label="视图编码"></el-table-column>
				<el-table-column prop="C_TEXT" label="视图名称"></el-table-column>
			...
			<el-dialog size="small" :title="title" :visible.sync="editDialog" :close-on-click-modal="false" width="70%" top="6vh">
				<dynamicViewEdit @closeAndFresh="closeFresh" ref="sourceInfo"></dynamicViewEdit>
				<div slot="footer" class="dialog-footer">
					<el-button @click="editDialog = false">取 消</el-button>
					<el-button @click="testView">测试</el-button>
...
<script>
	import {queryDynamicView,delDynamicView} from '../../api/api';
	import dynamicViewEdit from '@/components/system/dynamicViewEdit.vue';

	export default {
		components: {
			dynamicViewEdit:dynamicViewEdit
		},
		data() {
			return {
				tableHeight:'',
        ...
        this.searchTableData();		    
			...
			searchTableData() {
				...
				queryDynamicView(param).then(data => {
				...
				this.$confirm('此操作将删除该视图, 是否继续', '提示', {
		          ...
		          type: 'warning'
					...
					var params = {
			       	...
			       	delDynamicView(params).then((data)=>{
			       		this.$message({
				            message: '删除成功！',
				            type: 'success'
				        });
