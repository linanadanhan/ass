<template>
	<div>
		<el-form :model="addForm" ref="addForm" :rules="rules" label-width="80px">
				<el-row>
					<el-col :span="12">
						<el-form-item label="布局代码" prop="code">
						...
						<el-form-item label="布局包">
							<div class="up-file" style="height: 80px;">
								<div style="width: 400px;">
									<div style="float:left;width: 100px;height: 50px;">
										<el-upload class="avatar-uploader" :data="upLoadData" :action="fileUploadUrl" :show-file-list="false" :on-success="uploadSuccess" :before-upload="beforeAvatarUpload">
											<el-button slot="trigger" size="small" type="primary">选取布局包</el-button>
										</el-upload>
									</div>
								...
								<div style="height: 40px;">
									<img v-if="addForm.layoutImg" :src="addForm.layoutImg" class="avatar">
...
<script>
	import {
		saveLayout,isExitLayoutCode,getLayoutInfoById,importLayout
	} from '../../api/apiPortal';
		...
		data(){
			...
			var validateCodeUnique = (rule, value, callback) => {
		        if (value) {
		          ...
		          isExitLayoutCode({"code": value,"id":this.addForm.id}).then((data) => {
		              if (data) {
		                callback(new Error('布局代码已存在！'));
		              } else {
		                callback();
		              }
		          });
			...
			return{
				...
				isReadonly:false,
				fileUploadUrl: '/api/cos3-portal-manager/layout/importLayout',
		...
		methods:{
			loadData(id){
				...
				if(id == ''){//新增
					...
					getLayoutInfoById({id: id}).then((data) => { 
						this.addForm = data;
						this.addForm.referenceId = '';
						if (data.isImp != null && data.isImp != '' && data.isImp == '1'){
							this.addForm.layoutImg = gbs.portalFrontUrl + data.projectCode + '/layouts/'+data.code+'/bg.png';
						}
						this.$refs['addForm'].resetFields();
					});
				}
			...
			saveData(){
				this.$refs.addForm.validate((valid) => {
					if(valid) {
						saveLayout(this.addForm).then((data) => {
							this.$message({
					            message: '保存成功！',
					            type: 'success'
					        });
			...
			beforeAvatarUpload(file) {
				...
				if(!allow) {
					this.$message.error('上传布局包只能是 ZIP格式!');
				}
				if(!isLt2M) {
				...
				if (fileName.indexOf('.') != -1){
					...
					if (fileName != this.addForm.code){
						checkName = false;
						this.$message.error('上传布局包文件名与布局代码不一致!');
					}else{
