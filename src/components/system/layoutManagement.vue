        ...
        <el-input style="float: right;margin-top:0;margin-bottom:10px;width:150px" size="small" placeholder="请输入关键字" icon="search"
          ...
          <i slot="suffix" class="el-input__icon el-icon-search" @click="handleSearch"></i>
			...
			<el-table ref="multipleTable" :data="tableData" @row-click="rowClick" highlight-current-row border style="width: 100%" :height="tableHeight">
				...
				<el-table-column prop="C_CODE" label="布局代码"></el-table-column>
				<el-table-column prop="C_NAME" label="布局名称"></el-table-column>
          ...
          <template scope="scope">
			...
			<el-dialog size="small" :title="title" :visible.sync="editDialog" :close-on-click-modal="false" width="50%" top="8vh">
				<layoutForm @closeAndFresh="closeFresh" ref="layoutInfo"></layoutForm>
...
<script>
	import {queryLayoutDataTable,delLayout,importLayout,exportLayout  } from '../../api/apiPortal';
	import layoutForm from '@/components/system/layoutEdit.vue';
	...
	export default {
        ...
        this.searchTableData();
			...
			searchTableData() {
				...
				queryLayoutDataTable(param).then(data => {
				...
				this.$nextTick(() => {
   						 this.$refs.layoutInfo.loadData(row.C_ID,'');
   				});
			},rowClick(row){
			...
			addDataTable(){
				...
				this.editDialog = true;
				this.$nextTick(() => {
 					this.$refs.layoutInfo.loadData('');
 				});
			},deleteDataTable(index ,row){
		       	
				this.$confirm('此操作将永久删除该布局及其下所有引用的数据项, 是否继续', '提示', {
				...
				this.loading2 = true;
				exportLayout(params).then((data) => {
					this.loading2 = false;
					if (data.success){
						window.location.href = '/static/upload/zips/layouts/' + row.C_CODE + '.zip';
				        this.$message({
				          message: '导出成功',
				          type: 'success'
				        });
			  ...
			  this.loading2 = false;
				if (response.success){
					if (response.description == '1'){//布局已存在
						var layoutCode = response.title;
						this.$confirm('导入布局：'+layoutCode+'已存在，是否覆盖？', '提示', {
						  confirmButtonText: '确定',
						  cancelButtonText: '取消',
						  type: 'warning'
						}).then(() => {
				...
				if(!allow) {
					this.$message.error('导入布局包只能是 ZIP格式!');
				}
				if(!isLt2M) {
					this.$message.error('导入布局包大小不能超过 20MB!');
				}
		...
		mounted() {
			this.searchTableData();
			this.fileUploadUrl = '/api/cos3-portal-manager/layout/importLayout';
		},
	    created() {
