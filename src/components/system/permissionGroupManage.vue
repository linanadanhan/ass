	...
	<div class="con_main">
		<div class="childMain" style="width: 100%;">
			...
			<el-table ref="multipleTable" :data="tableData" v-loading.fullscreen.lock="loading2" @row-dblclick="dblclick" highlight-current-row border style="width: 100%;" :height="tableHeight">
				...
				<el-table-column prop="C_NAME" width="400" label="权限组名称"></el-table-column>
				<el-table-column prop="C_REMARK" label="权限组描述"></el-table-column>
			...
			<el-dialog size="small" :title="title" :visible.sync="editDialog" :close-on-click-modal="false" width="70%" top="6vh">
				<permissionGroupEdit @closeAndFresh="closeFresh" ref="sourceInfo"></permissionGroupEdit>
...
<script>
	import {queryPerssionGroup,delPerssionGroup} from '../../api/api';
	import permissionGroupEdit from '@/components/system/permissionGroupEdit.vue';
	...
	export default {
        ...
        this.searchTableData();		    
			...
			searchTableData() {
				...
				queryPerssionGroup(param).then(data => {
			...
			addDataTable(){
				...
				this.$confirm('删除权限组后，会将所有与权限组相关联的都删掉，确定要删除吗？', '提示', {
		          ...
		          type: 'warning'
					...
					var params = {
			       	...
			       	delPerssionGroup(params).then((data)=>{
			       		this.$message({
				            message: '删除成功！',
				            type: 'success'
				        });
