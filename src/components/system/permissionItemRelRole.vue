		...
		<el-row style="background:#f5f5f5;height:391px">
			...
			<el-col :span="9">
				<el-table ref="waitTable" :data="waitRole" @select="selectWaitRoleRow" @select-all="selectWaitRoleAll" border style="width: 100%" height="390">
					...
					<el-table-column prop="C_CODE" label="角色代码" width="120">
					</el-table-column>
			...
			<el-col :span="2">
				<div style="margin-top: 120px;">
					<el-button type="primary" @click="toGrant" icon="arrow-right" size="mini" style="margin-left:2px;">授权<i class="el-icon-arrow-right el-icon--right"></i></el-button>
				</div>
				<div style="margin-top: 60px;">
					<el-button type="primary" @click="cancelGrant" icon="el-icon-arrow-left" size="mini" style="margin-left:2px;">移除
					</el-button>
				</div>
			</el-col>
			<el-col :span="9">
...
<script>
	import {
		...
		getPermissionHasNoConnectRole,
		getPermissionHasConnectRole,
		savePermissionRelRole
		...
		methods: {
			...
			saveData() {
				...
				if(this.haveRole && this.haveRole.length > 0) {
					for(let role of this.haveRole) {
						selRole.push(role.C_ID)
					}
				}
				let param = {
					itemId: this.id,
				...
				savePermissionRelRole(param).then((data) => {
					this.$message({
						message: '保存成功！',
						type: 'success'
					});
