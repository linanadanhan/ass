				...
				<el-row>
					...
					<el-col :span="12">
						<el-form-item label="主题包">
							<div class="up-file" style="height: 100px;">
								<div style="width: 300px;">
									<div style="float:left;width: 100px;height: 50px;">
										<el-upload class="avatar-uploader" :data="upLoadData" :action="fileUploadUrl" :show-file-list="false" :on-success="uploadSuccess" :before-upload="beforeAvatarUpload">
											<el-button slot="trigger" size="small" type="primary">选取主题包</el-button>
										</el-upload>
									</div>
...
<script>
	import {
		saveTheme,isExitThemeCode,getThemeInfoById,importTheme
	} from '../../api/apiPortal';
		...
		data(){
			...
			var validateCodeUnique = (rule, value, callback) => {
		        if (value) {
		          ...
		          isExitThemeCode({"code": value,"id":this.addForm.id}).then((data) => {
		              if (data) {
		                callback(new Error('主题代码已存在！'));
		              } else {
		                callback();
		              }
		          });
			...
			return{
				addForm:{
					...
					projectCode:''				},
				isReadonly:false,
				fileUploadUrl: '/api/cos3-portal-manager/theme/importTheme',
		...
		methods:{
			loadData(id){
				if(id == ''){//新增
					...
					getThemeInfoById({id: id}).then((data) => { 
						this.addForm = data;
						this.addForm.referenceId = '';
						if (data.isImp != null && data.isImp != '' && data.isImp == '1'){
							this.addForm.themeImg = gbs.portalFrontUrl + data.projectCode + '/themes/'+data.code+'/theme.png';
						}
						this.$refs['addForm'].resetFields();
					});
				}
			...
			saveData(){
				this.$refs.addForm.validate((valid) => {
					if(valid) {
						saveTheme(this.addForm).then((data) => {
							this.$message({
			            message: '保存成功！',
			            type: 'success'
			        });
			...
			beforeAvatarUpload(file) {
				...
				if(!allow) {
					this.$message.error('上传主题包只能是 ZIP格式!');
				}
				if(!isLt2M) {
					this.$message.error('上传主题包大小不能超过 20MB!');
				}
				...
				if (fileName.indexOf('.') != -1){
					...
					if (fileName != this.addForm.code){
						checkName = false;
						this.$message.error('上传主题包文件名与主题代码不一致!');
					}else{
