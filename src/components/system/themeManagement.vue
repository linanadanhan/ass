	...
	<div class="con_main">
		<div class="childMain" style="width: 100%;">
			<div class="handle">
				...
				<!--<el-button type="primary" @click="test">测试保存缓存</el-button>
				<el-button type="primary" @click="get">测试获取缓存</el-button>-->
			...
			<el-table ref="multipleTable" :data="tableData" v-loading.fullscreen.lock="loading2" highlight-current-row @row-click="rowClick" border style="width: 100%" :height="tableHeight">
				<el-table-column type="selection" width="55" align="center"></el-table-column>
				<el-table-column prop="C_CODE" label="主题代码"></el-table-column>
				<el-table-column prop="C_NAME" label="主题名称"></el-table-column>
			...
			<el-dialog size="small" :title="title" :visible.sync="editDialog" :close-on-click-modal="false" width="50%" top="8vh">
				<themeForm @closeAndFresh="closeFresh" ref="themeInfo"></themeForm>
...
<script>
	import {queryThemeDataTable,delTheme,importTheme, exportTheme, saveBadgeInfo, getBadgeInfo} from '../../api/apiPortal';
	...
	export default {
		...
		methods: {
			...
			searchTableData() {
				...
				queryThemeDataTable(param).then(data => {
				...
				this.$nextTick(() => {
   						 this.$refs.themeInfo.loadData(row.C_ID,'');
   				});
			},rowClick(row){
			...
			addDataTable(){
				...
				this.editDialog = true;
				this.$nextTick(() => {
 					this.$refs.themeInfo.loadData('');
 				});
			},deleteDataTable(index ,row){
				
				this.$confirm('此操作将永久删除该主题及其下所有引用的数据项, 是否继续', '提示', {
				...
				this.loading2 = true;
				exportTheme(params).then((data) => {
					this.loading2 = false;
					if (data.success){
						window.location.href = '/static/upload/zips/themes/' + row.C_CODE + '.zip';
				        this.$message({
				          message: '导出成功',
				          type: 'success'
				        });
				...
				if (response.success){
					if (response.description == '1'){//主题已存在
						...
						this.$confirm('导入主题：'+siteCode+'已存在，是否覆盖？', '提示', {
						  confirmButtonText: '确定',
						  cancelButtonText: '取消',
						  type: 'warning'
						}).then(() => {
				...
				if(!allow) {
					this.$message.error('导入主题包只能是 ZIP格式!');
				}
				if(!isLt2M) {
					this.$message.error('导入主题包大小不能超过 20MB!');
				}
...
//		      		badge : 'test.a',
//		      		userId: 1,
//		      		json: '{"1":{"name":"测试1"},"2":{"name":"测试2"},"3":{"name":"测试3"}}'
//		      	};
//		      	
//		       	saveBadgeInfo(params).then((data)=>{
//		       		this.$message({
//			            message: '保存成功！',
//			            type: 'success'
//			        });
...
//		       	});
		...
		mounted() {
			this.searchTableData();
			this.fileUploadUrl = '/api/cos3-portal-manager/theme/importTheme';
		},
	    created() {
